function stem = stemmer(inString)
...
if k > 2
    ...
    if(strcmp(x{1},inString))
    x = step1c(x{1}, x{2}, k0); end;
    %disp(sprintf('Step 1C yields: %s', x{1}));
    if(strcmp(x{1},inString))
    x = step2(x{1}, x{2}, k0); end;
    %disp(sprintf('Step 2 yields: %s', x{1}));
    if(strcmp(x{1},inString))
    x = step3(x{1}, x{2}, k0); end;
    %disp(sprintf('Step 3 yields: %s', x{1}));
    if(strcmp(x{1},inString))
    x = step4(x{1}, x{2}, k0); end;
    %disp(sprintf('Step 4 yields: %s', x{1}));
    if(strcmp(x{1},inString))
    x = step5(x{1}, x{2}, k0); end;
...
switch b(k-1)
    case {'a'}
        if ends('al', b, k) flag = 1; end;
    case {'c'}
        if ends('ance', b, k) flag = 1; 
        elseif ends('ence', b, k) flag = 1; end;
    case {'e'}
        if ends('er', b, k) flag = 1; end;
    case {'i'}
        if ends('ic', b, k) flag = 1; end;
    case {'l'}
        if ends('able', b, k) flag = 1; 
        elseif ends('ible', b, k) flag = 1; end;
    case {'n'}
        if ends('ant', b, k) flag = 1; 
        elseif ends('ement', b, k) flag = 1; 
        elseif ends('ment', b, k) flag = 1; 
        elseif ends('ent', b, k) flag = 1; end;
    case {'o'}
        if ends('ion', b, k) 
            if j == 0
        ...
        elseif ends('ou', b, k) flag = 1; end;
    case {'s'}
        if ends('ism', b, k) flag = 1; end;
    case {'t'}
        if ends('ate', b, k) flag = 1; 
        elseif ends('iti', b, k) flag = 1; end;
    case {'u'}
        if ends('ous', b, k) flag = 1; end;
    case {'v'}
        if ends('ive', b, k) flag = 1; end;
    case {'z'}
        if ends('ize', b, k) flag = 1; end;
end
if measure(b, k0) > 1 & ~flag
    s4 = {b(k0:j), j};
else
    s4 = {b(k0:k), k};
end
